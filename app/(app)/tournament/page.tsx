...
function gdColor(goal_diff: number) {
  ...
  if (goal_diff < 0) return 'text-flame';
  return 'text-chalk';
}
            ...
            {groupKeys.map((label) => {
              ...
              return (
                <div key={label} className="overflow-hidden rounded-2xl border border-white/10 bg-pitch-900/60">
                  ...
                  {/* Column headers */}
                  <div className="grid grid-cols-[1.5rem_1fr_2rem_2rem_2rem_2rem_2.5rem] gap-x-1 border-b border-white/5 px-3 py-1.5 font-mono text-sm uppercase tracking-widest text-chalk">
                    <span />
                    <span>Team</span>
                  ...
                  {rows.map((row, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-center font-mono text-base text-chalk">{row.played}</span>
                        <span className="text-center font-mono text-base text-chalk">{row.won}</span>
                        <span className="text-center font-mono text-base text-chalk">{row.drawn}</span>
                  ...
                  <div className="flex items-center gap-1.5 border-t border-white/5 px-3 py-2">
                    ...
                    <span className="font-mono text-sm uppercase tracking-widest text-chalk">Qualify</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-pitch-900/60">
            ...
            {topScorers.length === 0 ? (
              <div className="flex h-28 items-center justify-center">
                <p className="font-mono text-base text-chalk">No goals scored yet</p>
              </div>
            ) : (
              topScorers.map((s, i) => (
                <div
                ...
                >
                  <span className="w-6 shrink-0 font-display font-black text-base text-chalk">{i + 1}</span>
                  {s.team_crest && (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {s.team_tla && (
                      <p className="font-mono text-base text-chalk">{s.team_tla}</p>
                    )}
                  </div>
                  <div className="flex items-baseline gap-1 shrink-0">
                    <span className="font-display font-black text-2xl text-lime">{s.goals}</span>
                    <span className="font-mono text-base text-chalk">{s.goals === 1 ? 'goal' : 'goals'}</span>
                  </div>
                </div>
              ))
            )}
              ...
              topAssisters.map((a, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {a.team_tla && (
                      <p className="font-mono text-base text-chalk">{a.team_tla}</p>
                    )}
                  </div>
                  <div className="flex items-baseline gap-1 shrink-0">
                    <span className="font-display font-black text-2xl text-lime">{a.assists}</span>
                    <span className="font-mono text-base text-chalk">{a.assists === 1 ? 'assist' : 'assists'}</span>
                  </div>
                </div>
              ))
            )}

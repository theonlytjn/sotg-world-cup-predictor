...
export default function ManualPredictionsPanel({
  ...
  return (
    <div>
      ...
      <div className="mt-4">
        <label className="block font-mono text-base uppercase tracking-widest text-chalk mb-1.5">
          Select fixture
        </label>
        <select
          value={fixtureId}
          onChange={(e) => setFixtureId(e.target.value ? Number(e.target.value) : '')}
          className="w-full max-w-md rounded-xl border border-white/15 bg-pitch-800 px-3 py-2 font-mono text-base text-chalk outline-none focus:border-lime/60"
        >
      ...
      {fixtureId && (
        <div className="mt-6">
          {loading ? (
            ...
            <>
              {selectedFixture && (
                <p className="mb-4 font-display text-base uppercase tracking-wide text-chalk">
                  ...
                  return (
                    <div key={p.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-pitch-900/60 px-4 py-3">
                      <div className="min-w-0 flex-1">
                        <span className="font-display text-base uppercase text-chalk">{p.display_name}</span>
                        {hasExisting && (
                          <span className="ml-2 font-mono text-base text-lime">existing</span>
                        )}
                      </div>
